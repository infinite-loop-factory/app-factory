...
export function buildPatterns(
  ...
  return allKeywords.map((kw) => {
    ...
    if (cjkScripts.includes(lang) || containsNonAscii(kw)) {
...
function buildInformationalPatterns(
  ...
  return patterns.map((p) => {
    if (containsNonAscii(p)) return new RegExp(escapeRegex(p), "i");
...
export function detectExtensions(prompt: string): string[] {
  ...
  for (;;) {
    const match = extPattern.exec(prompt);
    if (!match) break;
...
function shouldSkipPrompt(prompt: string): boolean {
  return !prompt.trim() || startsWithSlashCommand(prompt);
}
...
function matchWorkflow(
  workflow: string,
  def: TriggerConfig["workflows"][string],
  cleaned: string,
  prompt: string,
  lang: string,
  config: TriggerConfig,
  ...
  for (const pattern of patterns) {
    ...
    return buildContextLines(workflow, prompt, config).join("\n");
  }
...
function findWorkflowContext(
  ...
  for (const [workflow, def] of Object.entries(config.workflows)) {
    if (excluded.has(workflow)) {
      continue;
    }

    const context = matchWorkflow(
      workflow,
      def,
      cleaned,
      prompt,
...
function main() {
  ...
  if (handleDeactivation(prompt, lang, projectDir, sessionId)) {
    process.exit(0);
  }

  const cleaned = stripCodeBlocks(prompt);
  const context = findWorkflowContext(config, lang, cleaned, prompt);
  if (!context) {
    process.exit(0);
  }

  const workflow = context
    .match(/^\[OMA WORKFLOW: ([A-Z-]+)\]/m)?.[1]
    ?.toLowerCase();
  if (workflow && config.workflows[workflow]?.persistent) {
