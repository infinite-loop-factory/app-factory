...
import { getCurrentCoordinates } from "@/features/map/utils/native-location";
...
const MapGlobe = forwardRef<MapGlobeRef, MapGlobeProps>(
  ({ year, startDate, endDate }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      ...
      animateToUserLocation: async () => {
        const coordinates = await getCurrentCoordinates(Location);
        if (coordinates) {
          const newRegion = {
