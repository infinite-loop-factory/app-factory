...
export function Signin() {
  ...
  return (
    <View className={"flex w-fit gap-[12px]"}>
      ...
      <Button
        onPress={handleSubmit(async ({ email, password }) => {
          ...
          const select = await supabase.from("user").select("name");

          const data = select.data;
          if (res.error || !data?.[0]?.name) return alert("에러발생");

          setUser({ email, id: user.id, name: data[0].name });
        })}
      >
