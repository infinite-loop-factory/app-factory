...
### 1. 홈 화면 구성
- [ ] **기본 레이아웃**: 출발역(빈칸) + 도착역(빈칸). 가장 친숙하고 편리한 형태.
- [ ] **선택 영역**: 출발역(빈칸) / 도착역(빈칸) / 경유 추가(선택)
- [ ] **인터페이스 분리**: 각 영역(출발/도착/경유 추가) 탭 시 **서로 다른 전용 인터페이스**로 이동
  - 출발역(빈칸) 탭 → 출발역 전용 선택 인터페이스
  - 도착역(빈칸) 탭 → 도착역 전용 선택 인터페이스
  - 경유 추가 탭 → 경유역 전용 선택 인터페이스
