...
- 앱 시작 시 첫 화면
- 출발역/도착역 선택으로 이동
- GPS 기반 최근 역 자동 추천 (선택사항)

### 2. 역 선택 화면 (Station Selection Screen)
- 출발역 또는 도착역 선택
- 역 검색 기능
- 노선별 역 목록

### 3. 경로 결과 화면 (Route Result Screen)
- 경로 탐색 결과 표시
- 경로 요약 정보
- 경로 옵션 선택

### 4. 경로 상세 화면 (Route Detail Screen)
- 구간별 상세 정보
- 시각적 경로 표시
...
- 앱 로고/제목
- 출발역 선택 영역 (버튼 또는 카드)
  - GPS 사용 시: "현재 위치 근처 역" 자동 추천 표시
- 도착역 선택 영역 (버튼 또는 카드)
- "경로 찾기" 버튼 (출발/도착 모두 선택 시 활성화)
- GPS 토글 버튼 (위치 기반 추천 on/off)
- 최근 검색한 경로 (선택사항)

상태:
- 출발역 미선택: "출발역 선택" 표시
- 출발역 선택: 선택한 역명 표시, 변경 버튼
- GPS 활성화 시: "📍 강남역 (현재 위치에서 200m)" 형태 표시
- 도착역 미선택: "도착역 선택" 표시
- 도착역 선택: 선택한 역명 표시, 변경 버튼
```
