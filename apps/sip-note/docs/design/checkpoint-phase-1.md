...
- [x] `bun run --filter "@infinite-loop-factory/sip-note" type-check` — 신규 에러 0 (기존 SQLite 2 건은 무관)
- [x] `bun run --filter "@infinite-loop-factory/sip-note" test` — 23/23 passed (회귀 0)
- [x] `bun run --filter "@infinite-loop-factory/sip-note" lint` — Phase 1 신규 파일 0 warning. 기존 3건 (SQLite repo / migration kebab-case) 은 별도 `chore(sip-note): kebab-case rename` 으로 처리.
