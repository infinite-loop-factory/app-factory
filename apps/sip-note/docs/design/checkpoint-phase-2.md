...
| 트리거 | 발생 | 시점 | 발견된 불일치 |
|---|---|---|---|
| Phase 마무리 (의무) | ⏳ 대기 | 본 PR 머지 직후 시뮬레이터 9 컷 | (대기 중) |
| AI slop 6 종 의심 | ❌ 없음 | — | — |
| WCAG 자동 계산 ↔ 실제 화면 차이 | ❌ 없음 | — | — |
| 사용자 명시 요청 | ❌ 없음 | — | — |

**의무 캡처 plan**:
1. 지도 (다크) — 필터 비활성, 4 카테고리 핀 노출 + 위시리스트 핀 1
2. 지도 (라이트) — 필터 `bar` 활성
3. BottomSheet peek 25% (다크) — 카테고리 chip + 이름 + 방문 횟수
4. BottomSheet half 50% (다크) — peek + latestNote inline + CTA
5. BottomSheet half 50% (라이트) — 동일
6. PlaceDetail (다크) — hero `place.bar` + 노트 3 건
7. PlaceDetail (라이트) — hero `place.distillery` + 빈 상태 (addNote CTA)
8. PlaceDetail (다크) — hero `place.winery` + 노트 다수 (스크롤)
9. 홈 피드 + DayDivider (라이트) — caption brand-strong 스왑 검증
