...
| 트리거 | 발생 | 시점 | 발견된 불일치 |
|---|---|---|---|
| Phase 마무리 (의무) | 🟡 부분 자동화 (2026-05-30) | `e2e/flows/checkpoint-phase-2-screenshots.yaml` 로 9 컷 단일 flow 자동화. cut 6·7·8·9(place detail · 홈)는 기기 검증 완료, cut 1~5(지도·BottomSheet)는 AVD 의 GMS `MapsCoreDynamite` 모듈 파손으로 MapView crash → 보류 | _layout 강제-다크 재발화 버그(라이트 전환 불가) 발견·수정 |
| AI slop 6 종 의심 | ❌ 없음 | — | — |
...
1. 지도 (다크) — 필터 비활성, 4 카테고리 핀 노출 + 위시리스트 핀 1 — ⛔ 지도 env-block
2. 지도 (라이트) — 필터 `bar` 활성 — ⛔ 지도 env-block
3. BottomSheet peek 25% (다크) — 카테고리 chip + 이름 + 방문 횟수 — ⛔ 지도 env-block
4. BottomSheet half 50% (다크) — peek + latestNote inline + CTA — ⛔ 지도 env-block
5. BottomSheet half 50% (라이트) — 동일 — ⛔ 지도 env-block
6. PlaceDetail (다크) — hero `place.bar` + 노트 3 건 — ✅ 검증 (deep link `place/e2e-bar`)
7. PlaceDetail (라이트) — hero `place.distillery` + 빈 상태 (addNote CTA) — ✅ 검증
...
> **env-block (cut 1~5)**: Pixel_8 AVD 의 Google Play services `dl-MapsCoreDynamite` 모듈 파일 부재로
> `react-native-maps` `MapView` native init 가 앱을 죽인다 (test-plan §발견 이슈 #3 재발·악화).
> cold boot·GMS warm intent 로도 미복구. 결정적 시딩(`dev?seed`)·present 딥링크(`map?present`)·
> 테마 오버라이드(`dev?theme`)는 코드상 모두 동작 검증됨 — 정상 Maps AVD 에서 5 컷 즉시 채워짐.
...
- [x] `pnpm --filter sip-note exec tsc --noEmit` — Phase 2 신규 파일 0 에러. 기존 photo `storage.test.ts` 의 jest 타입 누락은 Phase 1 부터의 기존 이슈, 본 PR 무관
- [x] `pnpm --filter sip-note test` — 67/67 passed (회귀 0)
- [x] `pnpm --filter sip-note lint` — 0 warning (Biome No fixes applied)
