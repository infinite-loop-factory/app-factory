...
| 2026-06-21 (11회차) | Pixel_8 AVD (`emulator-5554`) + Metro(bun) | B6 geofence-deeplink 신규 작성 + 실행 | ✅ **B6 PASS** (전 step COMPLETED, 스크린샷 2컷). **환경 블로커 해소**: 기존 dev client(2026-05-10 빌드) 위에서 검정 화면 → 빨간 박스 `[Worklets] Mismatch ... (0.7.1 vs 0.5.1)`. **근본 원인**: `node_modules` 에 worklets 중복 — top-level `0.5.1`(APK 네이티브와 일치) + `reanimated/node_modules` 밑 stale `0.7.1`(JS 가 이걸 import). lockfile 은 `0.5.1` 단일이라 stale 잔여물. **해소**: `bun install --frozen-lockfile` 로 중복 제거 → JS=네이티브=0.5.1 일치 → `expo start --clear` 재번들 후 홈 정상 렌더. dev client 재빌드 불필요. **lesson**: ① `pnpm` 직접 호출 금지 — root `packageManager: bun@1.3.14` 라 `bun`/`bunx` 사용. ② maestro launchApp 만으로는 dev client 가 JS 를 못 받음 — Metro 기동 + `adb reverse tcp:8081 tcp:8081` 선행 필수. ③ worklets/reanimated 네이티브-JS 버전은 `node_modules` 중복 시 어긋남 → frozen install 로 정합화 |
...
| B6 | `geofence-deeplink.yaml` ✅ | 지오펜싱 알림 도착지 딥링크 — `place/e2e-wish`(위시리스트 ♥ + 빈 상태) + `place/e2e-bar`(바/펍 + 노트 3). ⚠️ `openLink` 는 expo-router 링킹이라 도착지 렌더만 검증 — `deep-link.ts` 알림 리스너 자체와 실 geofence enter 는 범위 밖(유닛 테스트 + EAS dev build 수동 검증 몫). 지도 비의존(기본 Pixel_8 가능) | Medium | ✅ PASS (2026-06-21, Pixel_8, 전 step COMPLETED + 스크린샷 `geofence-deeplink-wish.png`(♥ + 주소 + 빈 상태) / `geofence-deeplink-bar.png`(바/펍 + Lagavulin 16)) |
